...
class hifi_F16():
    def __init__(self, device=device):
        ...
        self.Cx_model.load_state_dict(torch.load(path + '/model/Cx.pth', map_location=device, weights_only=True))
        self.Cz_model = MLP(3, 1, [20, 10]).to(device=device)
        self.Cz_model.load_state_dict(torch.load(path + '/model/Cz.pth', map_location=device, weights_only=True))
        self.Cm_model = MLP(3, 1, [20, 10]).to(device=device)
        self.Cm_model.load_state_dict(torch.load(path + '/model/Cm.pth', map_location=device, weights_only=True))
        self.Cy_model = MLP(2, 1, [20, 10]).to(device=device)
        self.Cy_model.load_state_dict(torch.load(path + '/model/Cy.pth', map_location=device, weights_only=True))
        self.Cn_model = MLP(3, 1, [20, 10]).to(device=device)
        self.Cn_model.load_state_dict(torch.load(path + '/model/Cn.pth', map_location=device, weights_only=True))
        self.Cl_model = MLP(3, 1, [20, 10]).to(device=device)
        self.Cl_model.load_state_dict(torch.load(path + '/model/Cl.pth', map_location=device, weights_only=True))
        self.Cxq_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Cxq_model.load_state_dict(torch.load(path + '/model/Cxq.pth', map_location=device, weights_only=True))
        self.Czq_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Czq_model.load_state_dict(torch.load(path + '/model/Czq.pth', map_location=device, weights_only=True))
        self.Cmq_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Cmq_model.load_state_dict(torch.load(path + '/model/Cmq.pth', map_location=device, weights_only=True))
        self.Cyp_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Cyp_model.load_state_dict(torch.load(path + '/model/Cyp.pth', map_location=device, weights_only=True))
        self.Cyr_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Cyr_model.load_state_dict(torch.load(path + '/model/Cyr.pth', map_location=device, weights_only=True))
        self.Cnr_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Cnr_model.load_state_dict(torch.load(path + '/model/Cnr.pth', map_location=device, weights_only=True))
        self.Cnp_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Cnp_model.load_state_dict(torch.load(path + '/model/Cnp.pth', map_location=device, weights_only=True))
        self.Clp_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Clp_model.load_state_dict(torch.load(path + '/model/Clp.pth', map_location=device, weights_only=True))
        self.Clr_model = MLP(1, 1, [20, 10]).to(device=device)
        self.Clr_model.load_state_dict(torch.load(path + '/model/Clr.pth', map_location=device, weights_only=True))
        self.delta_Cx_lef_model = MLP(2, 1, [20, 10]).to(device=device)
        self.delta_Cx_lef_model.load_state_dict(torch.load(path + '/model/delta_Cx_lef.pth', map_location=device, weights_only=True))
        self.delta_Cz_lef_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cz_lef_model.load_state_dict(torch.load(path + '/model/delta_Cz_lef.pth', map_location=device, weights_only=True))
        self.delta_Cm_lef_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cm_lef_model.load_state_dict(torch.load(path + '/model/delta_Cm_lef.pth', map_location=device, weights_only=True))
        self.delta_Cy_lef_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cy_lef_model.load_state_dict(torch.load(path + '/model/delta_Cy_lef.pth', map_location=device, weights_only=True))
        self.delta_Cn_lef_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cn_lef_model.load_state_dict(torch.load(path + '/model/delta_Cn_lef.pth', map_location=device, weights_only=True))
        self.delta_Cl_lef_model = MLP(2, 1, [20, 10]).to(device=device)
        self.delta_Cl_lef_model.load_state_dict(torch.load(path + '/model/delta_Cl_lef.pth', map_location=device, weights_only=True))
        self.delta_Cxq_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Cxq_lef_model.load_state_dict(torch.load(path + '/model/delta_Cxq_lef.pth', map_location=device, weights_only=True))
        self.delta_Cyr_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Cyr_lef_model.load_state_dict(torch.load(path + '/model/delta_Cyr_lef.pth', map_location=device, weights_only=True))
        self.delta_Cyp_lef_model = MLP(1, 1, [20, 10, 5]).to(device=device)
        self.delta_Cyp_lef_model.load_state_dict(torch.load(path + '/model/delta_Cyp_lef.pth', map_location=device, weights_only=True))
        self.delta_Czq_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Czq_lef_model.load_state_dict(torch.load(path + '/model/delta_Czq_lef.pth', map_location=device, weights_only=True))
        self.delta_Clr_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Clr_lef_model.load_state_dict(torch.load(path + '/model/delta_Clr_lef.pth', map_location=device, weights_only=True))
        self.delta_Clp_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Clp_lef_model.load_state_dict(torch.load(path + '/model/delta_Clp_lef.pth', map_location=device, weights_only=True))
        self.delta_Cmq_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Cmq_lef_model.load_state_dict(torch.load(path + '/model/delta_Cmq_lef.pth', map_location=device, weights_only=True))
        self.delta_Cnr_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Cnr_lef_model.load_state_dict(torch.load(path + '/model/delta_Cnr_lef.pth', map_location=device, weights_only=True))
        self.delta_Cnp_lef_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Cnp_lef_model.load_state_dict(torch.load(path + '/model/delta_Cnp_lef.pth', map_location=device, weights_only=True))
        self.delta_Cy_r30_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cy_r30_model.load_state_dict(torch.load(path + '/model/delta_Cy_r30.pth', map_location=device, weights_only=True))
        self.delta_Cn_r30_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cn_r30_model.load_state_dict(torch.load(path + '/model/delta_Cn_r30.pth', map_location=device, weights_only=True))
        self.delta_Cl_r30_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cl_r30_model.load_state_dict(torch.load(path + '/model/delta_Cl_r30.pth', map_location=device, weights_only=True))
        self.delta_Cy_a20_model = MLP(2, 1, [20, 10, 10]).to(device=device)
        self.delta_Cy_a20_model.load_state_dict(torch.load(path + '/model/delta_Cy_a20.pth', map_location=device, weights_only=True))
        self.delta_Cy_a20_lef_model = MLP(2, 1, [20, 20, 10]).to(device=device)
        self.delta_Cy_a20_lef_model.load_state_dict(torch.load(path + '/model/delta_Cy_a20_lef.pth', map_location=device, weights_only=True))
        self.delta_Cn_a20_model = MLP(2, 1, [20, 10, 5]).to(device=device)
        self.delta_Cn_a20_model.load_state_dict(torch.load(path + '/model/delta_Cn_a20.pth', map_location=device, weights_only=True))
        self.delta_Cn_a20_lef_model = MLP(2, 1, [20, 20, 10]).to(device=device)
        self.delta_Cn_a20_lef_model.load_state_dict(torch.load(path + '/model/delta_Cn_a20_lef.pth', map_location=device, weights_only=True))
        self.delta_Cl_a20_model = MLP(2, 1, [20, 10]).to(device=device)
        self.delta_Cl_a20_model.load_state_dict(torch.load(path + '/model/delta_Cl_a20.pth', map_location=device, weights_only=True))
        self.delta_Cl_a20_lef_model = MLP(2, 1, [20, 20, 10]).to(device=device)
        self.delta_Cl_a20_lef_model.load_state_dict(torch.load(path + '/model/delta_Cl_a20_lef.pth', map_location=device, weights_only=True))
        self.delta_Cnbeta_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Cnbeta_model.load_state_dict(torch.load(path + '/model/delta_Cnbeta.pth', map_location=device, weights_only=True))
        self.delta_Clbeta_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Clbeta_model.load_state_dict(torch.load(path + '/model/delta_Clbeta.pth', map_location=device, weights_only=True))
        self.delta_Cm_model = MLP(1, 1, [20, 10]).to(device=device)
        self.delta_Cm_model.load_state_dict(torch.load(path + '/model/delta_Cm.pth', map_location=device, weights_only=True))
        self.eta_el_model = MLP(1, 1, [20, 10]).to(device=device)
        self.eta_el_model.load_state_dict(torch.load(path + '/model/eta_el.pth', map_location=device, weights_only=True))
